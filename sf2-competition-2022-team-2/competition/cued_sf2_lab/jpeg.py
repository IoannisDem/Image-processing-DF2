...
#from cued_sf2_lab.optimisation_functions import mse, nlevdwt, nlevidwt, img_size
...
def FDCT1(Y, step, rise1=1):
    fd_quan = [16, 11, 10, 16, 24, 40, 51, 61, 12, 12, 14, 19, 26, 58, 60, 55, 14, 13, 16, 24, 40, 57, 69, 56, 14, 17,
               22, 29, 51, 87, 89, 62, 18, 22, 37, 56, 68, 109, 103, 77, 24, 35, 55, 64, 81, 104, 113, 92, 49, 64, 78,
               87, 103, 121, 120, 101, 72, 92, 95, 98, 112, 100, 103, 99]
    fd = np.array(fd_quan)*step
    fd = np.reshape(fd, (8, 8))
    fd = np.matlib.repmat(fd, 32, 32)
    Yp = Y.copy()
    for i in range(256):
        for j in range(256):
            Yp[i, j] = quant1(Yp[i, j], fd[i, j], rise1 = fd[i, j]*rise1)
    return Yp

def FDCT2(Y,step, rise1=1):
    fd_quan = [16, 11, 10, 16, 24, 40, 51, 61, 12, 12, 14, 19, 26, 58, 60, 55, 14, 13, 16, 24, 40, 57, 69, 56, 14, 17,
               22, 29, 51, 87, 89, 62, 18, 22, 37, 56, 68, 109, 103, 77, 24, 35, 55, 64, 81, 104, 113, 92, 49, 64, 78,
               87, 103, 121, 120, 101, 72, 92, 95, 98, 112, 100, 103, 99]
    fd = np.array(fd_quan)*step
    ...
    for i in range(256):
        for j in range(256):
            Yp[i, j] = quant2(Yp[i, j], fd[i, j], fd[i, j])
    return Yp


def dc_sub(Y,N):
    Yp = Y.copy()
    for i in range(0, Yp.shape[0], N):
        # print('I',i)
        # take DC component from second image onwards
        for j in range(N, 256, N):
            # print('J',j)
            Yp[i, j] -= Yp[i, j - N]
    return Yp
...
def dc_recon(Y,N):
    ...
    for i in range(0, Yp.shape[0], N):
        # print('I',i)
        for j in range(256 - N, N - 1, -N):
            # print('X',j)
            Yp[i, j] += Yp[i, j - N]
    return Yp

def jpegenclbt(X: np.ndarray, qstep: float, N: int = 8, M: int = 8,
    ...
    t = np.s_[N // 2:-N // 2]  # N is the DCT size, I is the image size # slice(4,-4,None) == slice(start, end, step)
    Xp = X.copy()  # copy the non-transformed edges directly from X

    # Pre-filter X before applying DCT
    Xp[t, :] = colxfm(Xp[t, :], Pf)  # Apply Pf on the columns first (keeping only the sliced rows)
    Xp[:, t] = colxfm(Xp[:, t].T, Pf).T  # Apply Pf on the rows (keeping only the sliced cols)
...
def jpegdeclbt(vlc: np.ndarray, qstep: float, N: int = 8, M: int = 8,
...
def jpegenclbt1(X: np.ndarray, qstep: float, N: int = 8, M: int = 8,
        opthuff: bool = False, dcbits: int = 8, log: bool = True, s = 1.34
        ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def jpegdeclbt1(vlc: np.ndarray, qstep: float, N: int = 8, M: int = 8,
        bits: Optional[np.ndarray] = None, huffval: Optional[np.ndarray] = None,
        dcbits: int = 8, W: int = 256, H: int = 256, log: bool = True, s = 1.34
        ) -> np.ndarray:
